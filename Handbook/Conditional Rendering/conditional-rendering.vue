...
<!-- 
    ...
    toggle more than one element? In this case we can use v-if on a <template> element, which 
    serves as an invisible wrapper. The final rendered result will not include the <template> 
    element.
-->
...
<!-- 
    ...
    rendering from scratch. Beyond helping make Vue very fast, this can have some useful advantages. 
...
<!-- 
    Then switching the loginType in the code above will not erase what the user has already 
    entered. Since both templates use the same elements, the <input> is not replaced - just 
    its placeholder.

    This isn’t always desirable though, so Vue offers a way for you to say, “These two elements 
    are completely separate - don’t re-use them.” Add a key attribute with unique values:
-->
...
<!-- 
    Note that the <label> elements are still efficiently re-used, because they don’t have key 
    attributes.
